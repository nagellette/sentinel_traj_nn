...
input_name="$MODELINPUTS"

input_path_text="$${home_path}sentinel_traj_nn/model_config_files/"
input_path_text_all=""
for VARIABLE in $$input_name
do
  input_path_text_all="$$input_path_text_all$$input_path_text$$VARIABLE "
done
input_path_text="[$$input_path_text_all]"
...
python $${home_path}sentinel_traj_nn/run_model.py \
  ...
  $$input_path_text_all$${model_name} $${current_time} >> $${home_path}runtime_logs/$${new_fileName}
